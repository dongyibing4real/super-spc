...
- **Phase boundaries** (JMP-style): a horizontal header band sits above the plot area with phase labels in each strip. Alternating subtle gray backgrounds (even: 0.06, odd: 0.12 opacity) distinguish regions. Vertical gold divider lines (0.75px, opacity 0.30) extend from the header through the chart plot area. Sans-serif font, 10px, weight 600. Header band height scales with `vScale` (18px at full height). `computeLayout()` adds extra top padding when phases exist. Per-phase control limits render as independent line segments.
